...
## 2026-06-30 - Custom Toggle Switch Accessibility
**Learning:** Found that custom toggle switches (like the Biometric Authentication toggle) lacked explicit association with their text labels (`for` attribute) and helper descriptions (`aria-describedby`), making them difficult to use for screen reader users and reducing the click target area.
