...
## 2024-07-03 - Toggle Switch Accessibility
**Learning:** Custom toggle switches implemented with separate label text and helper descriptions failed to properly associate the text with the underlying `<input type="checkbox">`.
**Action:** Always link the text labels to the checkbox inputs using the `for` attribute and link helper descriptions using `aria-describedby` to ensure proper screen reader support and increase the click target area.
