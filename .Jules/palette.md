...
## 2025-07-04 - Accessible Custom Checkboxes and Toggles
**Learning:** In custom switch components, wrapping inputs in a label isn't always enough if the descriptive text lives in adjacent sibling elements.
**Action:** Always connect adjacent primary labels using the `for` attribute pointing to the input `id`, and link secondary helper text/descriptions using `aria-describedby` to ensure complete context for screen reader users and expand the click target area.
