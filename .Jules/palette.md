...
## 2025-05-15 - Toggle Switch Accessibility
**Learning:** Custom toggle switch patterns in the app (like Biometric Authentication and Lock Tag) lacked explicit connections between the input checkboxes, their adjacent labels, and helper descriptions. This reduced the clickable area and failed to announce crucial context to screen reader users.
**Action:** When implementing custom toggle switches, always connect the adjacent text labels to the checkbox inputs using the `for` attribute and link helper descriptions using `aria-describedby` to ensure proper screen reader support and increase the click target area.
