...
## 2024-04-18 - Optimize Unshift in Bulk Operations
**Learning:** Sequential `.unshift()` inside loops causes O(N^2) reallocation bottlenecks. Spread operators inside `.unshift(...newItems)` allow batching to O(N).
**Action:** Use batched prepends with spread arrays `data.tags.unshift(...[...createdTags].reverse())` when modifying large list states instead of sequential operations.
