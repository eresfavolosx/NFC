...
## 2026-06-19 - Array.unshift in loops causes O(N^2) memory reallocation
**Learning:** Sequential `unshift` operations inside loops create an O(N^2) bottleneck by shifting all existing elements on every iteration.
**Action:** Avoid `unshift` in loops. Instead, accumulate items and prepend them in a single batched O(N) operation using spread syntax (e.g., `arr.unshift(...newItems.reverse())`).
