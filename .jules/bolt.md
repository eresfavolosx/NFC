...
## 2024-05-18 - Caching Relational Lookups in Global Store
**Learning:** Rebuilding `Map` lookup structures (O(N) operations) inside view rendering functions or high-frequency event listeners (like keystrokes) creates severe bottlenecks, even if the lookup itself is O(1).
