...
## 2024-06-28 - Optimize sequential array unshifts
**Learning:** Using `Array.unshift()` inside a loop to prepend multiple items creates a severe O(N^2) memory reallocation bottleneck because it shifts all existing array elements on every iteration.
**Action:** When adding multiple items to the beginning of a large array, accumulate the new items in a temporary array, reverse them if replicating unshift order, and prepend them all at once using array concatenation (e.g., `[...newItems].reverse().concat(arr)`).
