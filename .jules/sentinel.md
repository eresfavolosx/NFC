...
## 2024-05-26 - Unescaped settings variables in Sidebar
**Vulnerability:** Found `brandLabel` (a user-controlled setting) interpolated directly into `innerHTML` inside `src/components/sidebar.js` without sanitization.
**Learning:** Always consider globally accessible settings configurations to be untrusted data and escape them when passing them to template strings.
**Prevention:** Wrap user-controlled configuration inputs loaded from the store with `escapeHTML` before rendering.
