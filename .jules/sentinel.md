...
## 2024-05-09 - Hardcoded Firebase API Key
**Vulnerability:** A hardcoded Firebase API key was present in `src/firebase.js` (`apiKey: "AIzaSyBuYtwyaiO0IJ42GZ8D6BdN9Ke5E-DcbLo"`).
**Learning:** While Firebase web client API keys are generally public identifiers, treating them as hardcoded secrets within this repository violates security guidelines and represents a critical vulnerability.
**Prevention:** Always use Vite environment variables (`import.meta.env.VITE_FIREBASE_API_KEY || ''`) for sensitive configuration keys instead of committing them directly to the source code.
