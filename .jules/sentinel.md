...
## 2025-06-30 - Fix hardcoded Firebase API key
**Vulnerability:** Found a hardcoded Firebase API key (`AIzaSy...`) in `src/firebase.js`. While Firebase Web API keys are generally safe for client bundles, hardcoding them violates best practices regarding secret management and prevents environment-specific configurations.
**Learning:** Configurations for external services like Firebase shouldn't be hardcoded into the source repository to prevent accidental leakage and inflexible environments.
**Prevention:** Always extract API keys and other configuration secrets to environment variables (e.g., `import.meta.env` or `process.env`). Use defensive programming (`typeof import.meta !== 'undefined'`) to prevent errors when testing in a Node environment versus building for the browser.
