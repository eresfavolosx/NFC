...
## 2025-07-05 - Fix Hardcoded Firebase API Key
**Vulnerability:** A hardcoded Firebase API key was found in src/firebase.js.
**Learning:** Storing secrets or API keys in source code is a critical vulnerability as it exposes them to anyone with access to the repository, leading to potential unauthorized access or abuse of the services.
**Prevention:** Always use environment variables (e.g., import.meta.env.VITE_FIREBASE_API_KEY) to manage sensitive information and configure deployment environments to inject these securely.
