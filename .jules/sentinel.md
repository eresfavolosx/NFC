...
## 2026-06-13 - Fix DOM XSS in Sidebar Brand Label
**Vulnerability:** Unescaped 'brandName' from persistent settings was injected directly into innerHTML in the sidebar component.
**Learning:** The assumption that admin settings are 'safe' led to a persistent XSS vulnerability when they are injected into the DOM without sanitization.
**Prevention:** Apply escapeHTML to all stored configuration parameters when rendering them into the UI.
