...
## 2026-06-04 - Unescaped Settings Variable in Sidebar (Stored DOM XSS)
**Vulnerability:** Found an unescaped variable (`settings.brandName`) being injected directly into the DOM via innerHTML in `src/components/sidebar.js`. This allowed a malicious user (or an attacker who compromised an admin account) to store a malicious script in the brand name setting, which would then execute for all users rendering the sidebar.
**Learning:** Application-wide configuration parameters (like global settings) are often incorrectly trusted, leading to missing sanitization when rendering UI components that use these settings.
**Prevention:** All fields loaded from persistent storage (like `store.settings`) into `innerHTML` must be treated as untrusted and wrapped with `escapeHTML`, regardless of the assumed privilege level of the data source.
