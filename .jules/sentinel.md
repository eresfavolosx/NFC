...
## 2025-05-26 - Hardcoded API Keys in Source Code
**Vulnerability:** Found a hardcoded Firebase API key in `src/firebase.js`. While Firebase web client API keys are generally public by design, this project strictly classifies hardcoded API keys in configuration files as critical vulnerabilities to prevent accidental exposure of privileged keys.
**Learning:** Development environments often start with hardcoded keys for convenience, which are accidentally committed to version control and not refactored out.
**Prevention:** Always use environment variables (e.g., `import.meta.env.VITE_FIREBASE_API_KEY`) for any API keys or secrets in the codebase, even those intended to be used on the client side.
