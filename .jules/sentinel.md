...
## 2025-06-01 - Fix Stored DOM XSS in Sidebar via unescaped settings
**Vulnerability:** Found unescaped template string interpolation of the 'brandName' setting (as 'brandLabel') in the Sidebar component. Since this setting is loaded from persistent storage, it enables a Stored DOM XSS attack vector if an attacker modifies the setting.
**Learning:** Global layout components like the sidebar are often overlooked for sanitization because they render application-wide configuration data, which developers incorrectly assume is safe.
**Prevention:** Apply the 'escapeHTML' utility to all dynamically interpolated strings in template literals, including application settings or variables sourced from persistent storage, regardless of the component's scope.
