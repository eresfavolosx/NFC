...
## 2025-05-26 - Hardcoded API Key Exposure in Firebase Config
**Vulnerability:** Found a hardcoded Firebase API key in `src/firebase.js`. While Firebase web keys are often meant to be public to clients, this repository considers it a critical vulnerability to hardcode keys directly in the codebase as it risks unintentional leakage to source control, which might later be scraped by attackers.
**Learning:** Hardcoded credentials or API keys (even public ones) in version-controlled files violate standard security practices, as they mix configuration with source code and bypass centralized secret management solutions like `.env` files.
**Prevention:** Always load configuration secrets and API keys from environment variables using `import.meta.env` (in Vite) or `process.env` (in Node.js). Never commit hardcoded keys to the source code repository. Ensure fallback handling (like `|| ''`) exists to prevent crashes in environments where the key might be temporarily missing.
