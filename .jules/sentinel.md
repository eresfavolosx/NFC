...
## 2025-05-26 - Unescaped Tag Location Fields (Stored XSS)
**Vulnerability:** Found unescaped template string interpolations of user-controlled inputs (`tag.location.lat`, `tag.location.lng`) within the tags view in `src/views/tags.js`.
**Learning:** Secondary or deeply nested object properties are frequently missed when applying escaping to primary fields. Data loaded from persistent storage, even complex structures like geolocation, must be treated as untrusted.
**Prevention:** Sanitize all nested properties of objects interpolated into HTML strings by wrapping them in `escapeHTML()`.
