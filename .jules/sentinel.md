...
## 2025-10-27 - DOM-based escapeHTML vulnerable to attribute breakout
**Vulnerability:** Found that the `escapeHTML` utility in `src/utils.js` was using `div.textContent` and `div.innerHTML` to escape strings. This approach does not escape quotation marks (`"` and `'`), making it vulnerable to XSS if the escaped output is used within HTML attributes (e.g., `<input value="${escapeHTML(val)}">`).
**Learning:** Using DOM properties to "escape" HTML is insufficient because it's designed to make text safe for rendering as text nodes, not for attribute interpolation.
**Prevention:** Always use robust, regex-based escaping (like the one in `src/utils/sanitize.js`) or established libraries (like DOMPurify) that correctly handle all HTML metacharacters including quotes.
