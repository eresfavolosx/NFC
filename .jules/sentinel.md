...
## 2026-05-10 - Extracted hardcoded Firebase API keys to environment variables
**Vulnerability:** Found a hardcoded `apiKey` in `src/firebase.js`.
**Learning:** Hardcoded configuration values, especially API keys, violate security best practices as they can be easily extracted from source control.
**Prevention:** Utilize environment variables (e.g., `import.meta.env`) combined with a `.env.example` file to securely manage and document configuration secrets outside of the codebase.
