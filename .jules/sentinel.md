...
## 2026-05-07 - Remove Hardcoded Firebase API Key
**Vulnerability:** Found a hardcoded Firebase API key in `src/firebase.js`. This allowed the API key to be committed into the repository, posing a risk of unauthorized access or misuse of Firebase services.
**Learning:** Hardcoded secrets in client-side configuration files often occur when initializing third-party services (like Firebase), as developers may accidentally leave the copied config block unparameterized.
**Prevention:** Always use environment variables (e.g., `import.meta.env.VITE_FIREBASE_API_KEY`) to supply secrets or configuration at build time rather than hardcoding them in the source code.
