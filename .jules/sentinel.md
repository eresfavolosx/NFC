...
## 2024-05-24 - Stored DOM XSS in Sidebar Component
**Vulnerability:** Unescaped interpolation of untrusted `brandName` setting in `src/components/sidebar.js`.
**Learning:** Application configuration data and variables sourced from persistent storage (like `store.settings`) must be treated as untrusted user input, especially when dynamically interpolated into template literals assigned to `innerHTML`.
**Prevention:** Always apply the `escapeHTML` utility when dynamically interpolating these values to prevent Stored DOM XSS. Do not assume data is safe simply because it originates from a settings store.
