...
**Vulnerability:** Found a hardcoded Firebase API key in `src/firebase.js`. While Firebase web API keys are technically public, hardcoding them directly in the repository is a bad practice, violates coding standards, and classifies as a critical vulnerability.
**Learning:** Configuration files are a common place to find accidentally committed secrets. All secrets, even those meant for public client access, should be injected via environment variables.
**Prevention:** Always use environment variables (e.g., `import.meta.env.VITE_FIREBASE_API_KEY`) for API keys and secrets in frontend configuration files to prevent them from being checked into version control. Avoid optional chaining (`?.`) when referencing `import.meta.env` as Vite's static string replacement process may not handle it correctly.
