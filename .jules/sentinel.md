...
## 2026-06-03 - [Stored XSS via Persistent Settings]
**Vulnerability:** Untrusted user input loaded from `store.settings.brandName` was directly interpolated into the sidebar's `innerHTML` without sanitization, leading to stored DOM XSS.
...
**Prevention:** Ensure all persistent configuration data displayed in the UI is wrapped in `escapeHTML` before interpolation.
