...
## 2026-06-18 - Fix DOM-based escapeHTML vulnerability
**Vulnerability:** Found a vulnerable DOM-based `escapeHTML` implementation in `src/utils.js` that relies on `div.innerHTML`, which fails to properly escape quotes (like `'` and `"`). This can lead to XSS vulnerabilities when the escaped output is used in attributes or specific contexts.
**Learning:** Implementing security functions like `escapeHTML` using the DOM (`innerHTML`) is unsafe and error-prone. Always use a secure, regex-based character replacement implementation.
**Prevention:** Replace the unsafe implementation by importing and re-exporting the secure, regex-based `escapeHTML` utility from `src/utils/sanitize.js`.
