...
## 2024-06-05 - Stored DOM XSS in Sidebar
**Vulnerability:** The sidebar component retrieved `store.settings.brandName` (which is stored in localStorage) and directly interpolated it into `sidebar.innerHTML` without escaping, resulting in a stored DOM XSS vulnerability.
**Learning:** Application settings loaded from persistent storage must be treated as untrusted input and sanitized with `escapeHTML` before being interpolated into `innerHTML`, just like any other user input.
**Prevention:** Always wrap data retrieved from `store.settings` in `escapeHTML` when used in template strings assigned to `innerHTML`.
