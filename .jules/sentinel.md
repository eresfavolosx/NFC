...
## 2024-05-24 - Stored DOM XSS via unescaped Settings interpolation
**Vulnerability:** The application was vulnerable to stored DOM XSS in the sidebar because the `brandName` setting, loaded from persistent storage, was directly interpolated into `sidebar.innerHTML` without sanitization.
**Learning:** Even internal configuration or settings loaded from local storage must be treated as untrusted user input, especially when the application provides UI to mutate these settings (e.g., in `src/views/settings.js`).
**Prevention:** Always apply `escapeHTML` to any dynamic data—including user preferences or application state—before interpolating it into HTML template strings assigned to `innerHTML`.
