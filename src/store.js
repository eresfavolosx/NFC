  ...
  get stats() {
    if (!this._cache.stats) {
      ...
      this._cache.stats = {
        ...
        // ⚡ Bolt: Replace .filter(...).length with .reduce(...) to prevent intermediate array allocation.
        // Why: Reduces GC overhead by avoiding intermediate array creation on large datasets.
