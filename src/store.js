...
export const store = {
  ...
  // ── Links CRUD ──
  get links() { 
    // ⚡ Bolt: Cache array filters to prevent O(N) re-calculation per property access
    // Cache is safely invalidated globally in `_notify()` on any data mutation.
    this._cache = this._cache || {};
    if (!this._cache.links) {
      this._cache.links = this.isSuperAdmin()
        ? [...data.links]
        : data.links.filter(l => l.ownerEmail === this.user?.email || !l.ownerEmail);
    }
  ...
  get tags() { 
    ...
    if (!this._cache.tags) {
      ...
      this._cache.tags = this.isSuperAdmin()
        ? [...data.tags]
        : data.tags.filter(t => t.ownerEmail?.toLowerCase() === userEmail || !t.ownerEmail);
    }
  ...
  // ── Client Discovery ──
  get allClientEmails() {
    // ⚡ Bolt: Cache unique set computation to prevent O(N) array traversals
    // Cache is safely invalidated globally in `_notify()` on any data mutation.
    this._cache = this._cache || {};
  ...
  // ── Stats ──
  get stats() {
    // ⚡ Bolt: Cache statistics object creation to prevent repeated reduce operations
    // Cache is safely invalidated globally in `_notify()` on any data mutation.
    this._cache = this._cache || {};
