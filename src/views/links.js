...
function linkFormContent(link = null) {
    ...
    <div class="form-group">
      <label class="form-label" for="linkTitle">Title <span aria-hidden="true" style="color: var(--color-error); margin-left: 2px;">*</span></label>
    ...
    <div class="form-group">
      <label class="form-label" for="linkUrl">URL <span aria-hidden="true" style="color: var(--color-error); margin-left: 2px;">*</span></label>
...
function renderLinkCard(link, assignedTags, index, t) {
    ...
    const searchString = escapeHTML((link.title + ' ' + link.url).toLowerCase());

    return `
    <div class="link-card card animate-fade-up" style="animation-delay: ${0.05 * index}s" data-id="${link.id}" data-search="${searchString}" data-category="${escapeHTML(link.category)}">
